    ...
    <div className="space-y-6">
      ...
      {talents.length === 0 ? (
        <div className="panel-soft">
          <EmptyState
            icon={Radar}
            title="Скрытых талантов не найдено"
            description="Все кандидаты с высоким потенциалом уже находятся в шорт-листе или ожидают оценки."
          />
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {talents.map((talent) => (
            <div key={talent.id} className="group panel-soft grain p-5 transition hover:border-brand-green/25">
              <Link href={`/dashboard/candidates/${talent.id}`} className="block">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className="text-2xl font-black text-white">{talent.final_score}</span>
                ...
                {talent.tags.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {talent.tags.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full border border-brand-green/20 bg-brand-green/8 px-2.5 py-1 text-[10px] font-semibold text-brand-green"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              {/* Invite action — outside Link so click doesn't navigate */}
              <div className="mt-3 border-t border-white/6 pt-3">
                <InviteButton candidateId={talent.id} currentStatus={talent.status} />
              </div>
            </div>
          ))}
        </div>
